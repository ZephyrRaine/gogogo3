...
func on_launch_done(_a, _b):
	pebble = null
	launch_hud.set_total_score_visible(true)
	launch_hud.display_score(_a * _b, _b, _a)

func _input(event: InputEvent) -> void:
	if final_score_displayed && event.is_action_pressed("interact"):
		launch_hud.set_score_visible(false)
...
func _process(_delta: float) -> void:
	if pebble:
		launch_hud.display_score(0, pebble.score_bounces, pebble.score_distance)
...
func display_score(distance: float, bounces: int):
	...
	launch_hud.display_score(round_score, multiplier, final_speed_score)
